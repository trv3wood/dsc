 ...
 *    Number of pixels per group
 * \return
 *    0 = success; 1 = possible overflow
 ************************************************************************
 */
int check_qp_for_overflow(dsc_cfg_t *dsc_cfg, int pixelsPerGroup)
{
	double min_target_bpg;
	int p_mode_bits, max_bits;
	int cpnt, max_res_size;
	int cpntBitDepth[NUM_COMPONENTS];
	int fullGroupsPerLine;
	int adjustedSliceWidth;
	double unbudgetedBitsPerLine;

	adjustedSliceWidth = (dsc_cfg->native_420 || dsc_cfg->native_422) ? (dsc_cfg->slice_width / 2) : dsc_cfg->slice_width;
	...
	fullGroupsPerLine = adjustedSliceWidth / pixelsPerGroup;
	unbudgetedBitsPerLine = 0;
	if (adjustedSliceWidth % pixelsPerGroup)
		unbudgetedBitsPerLine = (bitsPerPixel * (pixelsPerGroup - (adjustedSliceWidth % pixelsPerGroup)));
	
	// These bits have to be made up over the course of the slice line
	min_target_bpg -= unbudgetedBitsPerLine / fullGroupsPerLine;
	...
	// Followed by all-0-residual predicted group (where predicted size is max size-1)
	...
	max_bits += p_mode_bits;
	if (generateRcParameters < 2 && (double)max_bits > min_target_bpg * 2)
	{
...
#endif
{
	...
	printf("Display Stream Compression (DSC) reference model version 1.67\n");
	...
	while ((strlen(infname)>0) || !feof(list_fp) || multiFrameYuvFile)
	{
		...
		if (dsc_codec.convert_rgb)
		{
			...
			for (tpidx=0; tpidx<2; ++tpidx)
			{
				...
				temp_pic[tpidx] = (pic_t *)pcreate_ext(FRAME, YUV_HD, YUV_444, dsc_codec.pic_width, dsc_codec.pic_height, dsc_codec.bits_per_component);
				temp_pic[tpidx]->bits = dsc_codec.bits_per_component;
		...
		if (function == 2 && !ip)
		{
			// Try to read .ref pic if present to use as a PSNR reference
			if (dpxFileOutput || hdrDpxFileOutput)
			{
				strcpy(f, fn_o);
#ifdef WIN32
				...
				strcat(f, base_name);
				strcat(f, ".ref.dpx");
				type = hdr_dpx_determine_file_type(f);
				if (type == 1 || type == 2)
				{
					if (dpx_read(f, &ref_pic, dpxRPadEnds, dpxRForceBe, dpxRDatumOrder, rbSwap))
					{
						// Read failed - that's ok, we just won't do PSNR
						if (ref_pic)
							pdestroy(ref_pic);
						ref_pic = NULL;
					}
				...
				{
					if (hdr_dpx_read(f, &ref_pic, &hdrdpx_f, &hdrdpx_user, &hdrdpx_sbm))
					{
						// Read failed - that's ok, we just won't do PSNR
						if (ref_pic)
							pdestroy(ref_pic);
			...
			if (!ip && ppmFileOutput && dsc_codec.convert_rgb == 1)
			{
				strcpy(f, fn_o);
#ifdef WIN32
				strcat(f, "\\");
			...
			{	
				...
				strcat(f, base_name);
				strcat(f, ".ref.yuv");
				if (yuv_read(f, &ref_pic, dsc_codec.pic_width, dsc_codec.pic_height, frameNumber, dsc_codec.bits_per_component, &num_frames, yuvFileFormat))
				{
					// Read failed - that's ok, we just won't do PSNR
					if (ref_pic)
						pdestroy(ref_pic);
			...
			// Ref needs to match output format exactly for PSNR computation to be valid
			if (ref_pic && (ref_pic->color != op_dsc->color || ref_pic->chroma != op_dsc->chroma || ref_pic->bits != op_dsc->bits))
			{
				pdestroy(ref_pic);
				ref_pic = NULL;
			}
		...
		if (dsc_codec.native_422 || dsc_codec.native_420)
			fprintf(logfp, "\n%2.2f bits/(2-pixel pair), %d bits/component,", dsc_codec.bits_per_pixel / 16.0, dsc_codec.bits_per_component);
		else
