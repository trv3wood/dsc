...
void print_pps(FILE *logfp, dsc_cfg_t *dsc_cfg)
{
    ...
    fprintf(logfp, "first_line_bpg_offset    %5d\n", dsc_cfg->first_line_bpg_ofs);
...
void print_pps_v2(FILE *logfp, dsc_cfg_t *dsc_cfg)
{
	...
	PRINT_PPS_VAL("first_line_bpg_offset", "0x%02x", dsc_cfg->first_line_bpg_ofs, 1);
	...
	if (dsc_cfg->dsc_version_minor == 2)
	{
		...
		PRINT_PPS_VAL("second_line_bpg_offset", "0x%02x", dsc_cfg->second_line_bpg_ofs, 1);
		PRINT_PPS_VAL("nsl_bpg_offset", "0x%04x", dsc_cfg->nsl_bpg_offset, 0);
		fprintf(logfp, "=> %f\n", dsc_cfg->nsl_bpg_offset/2048.0);
		PRINT_PPS_VAL("second_line_offset_adj", "0x%04x", dsc_cfg->second_line_ofs_adj, 1);
	}
}
