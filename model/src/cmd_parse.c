...
static size_t* distinguish_dist(cmdarg_t *cmdargs, int *order, int num_args)
{
  size_t* distinguish;
  int i;
  size_t c;
  ...
  distinguish = calloc(num_args, sizeof(size_t));

  distinguish[order[0]] = 0;
  for (i=0; i<num_args-1; i++) {
    size_t minlng = MIN(strlen(cmdargs[order[i]].cmd), strlen(cmdargs[order[i+1]].cmd));
...
static int test_cmd(char *arg, char **v, const cmdarg_t cmdarg, const size_t distinguish)
{
  int i;
  *v = NULL;
  for (i=0; 1; i+=1) {
    if ('\0' == arg[i]) {
      int passed_test = (int)distinguish < i;
      return passed_test;
    }
    ...
    if (0 == (*val_r)[0]) {
      *val_r = NULL;
	  ...
	  //*past_val = 0; // terminate after value
	    char* past_val = *val_r + strcspn(*val_r, "\n");
		while (past_val != *val_r && (*past_val == ' ' || *past_val == '\r' || *past_val == '\n' || *past_val == '\t' || *past_val == '\0'))
			past_val--;
		past_val++;
		*past_val = 0;
    }
...
void parse_cmd_usage(cmdarg_t *cmdargs)
{
  ...
  while (NULL != cmdargs[num_args].var_ptr) {
    size_t lng = strlen(cmdargs[num_args].cmd);
  ...
  for (i=num_args-1; i>=0; i--) {
    size_t len = strlen(cmdargs[order[i]].cmd);
    if (len-1 <= distinguish[order[i]]) {
      strcpy(cmd, cmdargs[order[i]].cmd);
    }
    else {
      int j;
      for (j=0; j<(int)len; j+=1) {
        int k = (j<=(int)distinguish[order[i]])? j: j+1;
